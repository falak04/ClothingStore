...
import { addSellerProduct } from '../api/api';
import { useNavigate } from 'react-router-dom';
// import Navbar from '../components/Navbar';

export default function AddSellerProduct() {
  const [form, setForm] = useState({ name: '', price: '', description: '', category: '', Gender: '', imageUrl: '', size: [], color: [], stock: 0, isFeatured: false });
  ...
  const handleSubmit = async e => {
    ...
    setSuccess('');
    try {
      await addSellerProduct({ ...form, variants, sellerId: user._id });
  ...
  return (
    <>
      ...
      <div className="max-w-xl mx-auto mt-10 p-6 bg-white rounded-xl shadow border border-pale-pink">
        <h2 className="text-2xl font-bold mb-4 text-classic-pink">Add Product</h2>
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <button type="submit" className="bg-classic-pink hover:bg-pale-pink text-white hover:text-classic-pink py-2 rounded transition">Add Product</button>
        </form>
      </div>
    </>
  );
